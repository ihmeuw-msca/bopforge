...
#     signal[mask_middle] = get_signal(signal_model, risk[mask_middle])
#     # Extrapolate: use point-slope to have smooth continuity
#     # Calculate slope at the boundary of data-supported region
#     if mask_left.any():
#         slope_left = (signal_from_data[1] - signal_from_data[0]) / (
#             risk_from_data[1] - risk_from_data[0]
#         )
#         y_anchor_left = signal[mask_middle][0]
#         signal[mask_left] = y_anchor_left + slope_left * (
#             risk[mask_left] - risk[mask_middle][0]
#         )
#     if mask_right.any():
#         # Point-slope using the last two points of the data-supported signal
#         slope_right = (signal_from_data[-1] - signal_from_data[-2]) / (
#             risk_from_data[-1] - risk_from_data[-2]
#         )
#         # Anchor the line precisely at the last point of the middle segment
#         y_anchor_right = signal[mask_middle][-1]
#         signal[mask_right] = y_anchor_right + slope_right * (
#             risk[mask_right] - risk[mask_middle][-1]
#         )
#     # If left bound is truncated, get original signal precisely at truncated risk value
#     # and offset curve appropriately
#     if (
#         settings["draws"]["risk_lower"] is not None
#         and settings["draws"]["risk_lower"] > data_risk_lower
#     ):
#         truncated_left_bound = settings["draws"]["risk_lower"]
#         risk_with_truncated_bound = np.sort(
#             np.append(risk_from_data, truncated_left_bound)
#         )
#         idx = np.where(risk_with_truncated_bound == truncated_left_bound)[0][0]
#         signal_at_truncated_bound = get_signal(
#             signal_model, risk_with_truncated_bound
#         )[idx]
#         signal += signal_at_truncated_bound
